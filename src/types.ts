...
export interface NumerologyCoreNumber {
  key: string;         // lifePath, destiny, soul, personality, maturity, balance
  label: string;       // Số Chủ Đạo, Số Sứ Mệnh...
  value: number;
  keywords: string[];  // 3-4 keywords mô tả
  description: string; // 2-3 câu mô tả chi tiết
}
...
export interface NumerologyLifeCycle {
  number: number;
  name: string;          // GIEO HẠT, CHÍN, THU HOẠCH
  label: string;         // "Chu kỳ 1"
  ageRange: string;      // "Đầu đời - 29 tuổi"
  yearRange: string;     // "(2026 - 2055)"
  linkedNumber: number;  // số liên kết
  description: string;
...
export interface NumerologyProfile {
  ...
  // Tab 3: Dự báo
  personalYears: NumerologyPersonalYear[]; // 3 năm: hiện tại + 2 tiếp theo
